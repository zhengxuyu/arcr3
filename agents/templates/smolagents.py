...
class SmolCodingAgent(LLM, Agent):
    ...
    def main(self) -> None:
        ...
        model = OpenAIServerModel(self._model)
    ...
    def _execute_action(self, action: GameAction, action_description: str = "") -> str:
        ...
        if frame := self.take_action(action):
            ...
            logger.info(
                f"{self.game_id} - {action.name}: count {self.action_counter}, score {getattr(frame, 'score', frame.levels_completed)}, avg fps {self.fps})"
            )
