...
class ReasoningActionResponse(BaseModel):
    ...
    name: Literal["ACTION1", "ACTION2", "ACTION3", "ACTION4", "RESET"] = Field(description="The action to take.")
    reason: str = Field(
        description="Detailed reasoning for choosing this action",
        min_length=10,
        max_length=2000,
    )
    short_description: str = Field(description="Brief description of the action", min_length=5, max_length=500)
...
class ReasoningAgent(ReasoningLLM):
    ...
    def generate_grid_image_with_zone(self, grid: List[List[int]], cell_size: int = 40) -> bytes:
...
- The door has a pink border and a shape inside.
    ...
    def choose_action(self, frames: List[FrameData], latest_frame: FrameData) -> GameAction:
        ...
        reasoning_meta = {
            ...
            "response_preview": (
                action_response.reason[:200] + "..." if len(action_response.reason) > 200 else action_response.reason
            ),
