...
When you run an agent, a `.recording.jsonl` file is written under `recordings/` (or `RECORDINGS_DIR`). You can turn it into an animated **GIF** or **MP4** video with step, state, action, and levels shown on each frame.

**List recordings:**

```bash
uv run python scripts/visualize_recording.py
```

**Generate GIF (default):**

```bash
uv run python scripts/visualize_recording.py recordings/<filename>.recording.jsonl
```

Writes `<filename>.gif`. Use `-o out.gif` to set the path; `--duration 400` for ms per frame.

**Generate MP4** (requires `pip install imageio[ffmpeg]` and ffmpeg):

```bash
uv run python scripts/visualize_recording.py recordings/<filename>.recording.jsonl -o out.mp4 --format mp4 --fps 2.5
```
