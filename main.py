...
HEADERS = {
    ...
    "User-Agent": "Mozilla/5.0 (compatible; ARC-AGI-3-Agent/1.0)",
}
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Override LLM model (e.g. gpt-4o-mini, openai/gpt-4o, anthropic/claude-3.5-sonnet when using OpenRouter).",
    )
    ...
    if args.game:
        ...
        games = [gid for gid in full_games if any(gid.startswith(prefix) for prefix in filters)]
    ...
    if not games:
        if full_games:
            ...
            logger.error("No games available to play. Check API connection or recording file.")
        return
